...
self.addEventListener('install', (event) => {
  event.waitUntil((async () => {
    ...
    // If ANY URL in addAll() 404s, the whole install fails and the SW never
    // activates (which makes tools like PWABuilder report "no service worker").
    // So we try addAll(), but fall back to per-asset caching if needed.
    try {
